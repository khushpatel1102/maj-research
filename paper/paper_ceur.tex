%% =====================================================================
%%  CEUR-WS single-column version for ELMKE 2026 (ISWC)
%%  "When Does Memory Help an LLM Judge?"
%%  Converted from the ACL two-column source (paper.tex).
%% =====================================================================
...
\title{When Does Memory Help an LLM Judge? A Provably Leakage-Free,
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{figures/accuracy_ci.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{figures/structure_ablation.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{figures/poisoning_curve.png}
...
The experiments in this and the following two subsections are a later, independent run, so their stateless baseline is 66.2--68.1\% rather than the 70.0\% of \S\ref{sec:results}. That $\approx4$-point drift is not a discrepancy to explain away; it sits inside the $\approx5$-point cross-seed band we already reported (\S\ref{sec:results}), and it is worth stating plainly because it is the same stability the paper argues for, showing up again: the baseline wobbles by a few points between runs, and so does everything measured against it, which is exactly why every claim here is a \emph{paired} comparison on identical items rather than a difference of two headline numbers.

A null result invites an uninteresting explanation: perhaps the judge simply cannot read retrieved memory, so \emph{no} memory content would ever move its verdict. We rule this out with a memory-use control experiment. Holding the prompt template fixed, we replace the retrieved block with (i) a \emph{cheating oracle}: the test item's own ground-truth verdict injected as an exemplar; and three negative controls: (ii) a random unrelated attempt, (iii) label-shuffled exemplars, and (iv) a topically irrelevant block token-matched to the oracle block. All four route through the identical memory prompt, so any accuracy change is attributable to the injected content alone.
...
\textbf{Cheating oracle} & \textbf{78.8} & \textbf{60.0} & 15.0 \\
...
\caption{Memory-use controls. Only the cheating oracle (informative memory) clears the stateless baseline band; the three uninformative controls fall inside it. Bars are accuracy with Wilson 95\% CIs; the shaded band is the stateless CI.}
\label{fig:controls}
\end{figure}

Table~\ref{tab:controls} and Figure~\ref{fig:controls} show a clean dissociation. When memory carries the answer, accuracy rises $+12.5$ points, concentrated on failing items (fail-recall $35\to60$); per item, the oracle flips eleven verdicts from wrong to right and only one the other way. The three uninformative controls stay within $\pm3$ points of the no-memory baseline. \emph{The judge is fully capable of using memory when memory is informative}; the null elsewhere is therefore a property of the benchmark's memory carrying no verdict-relevant signal, not of a memory-blind judge. This is the positive half of the bracket that Proposition~\ref{prop:bayes} predicts: accuracy moves exactly when the injected content changes $P(Y\mid X,M)$, and not otherwise.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.86\linewidth]{figures/cv_deltas.png}
\caption{Grouped five-fold CV ($n{=}160$): paired accuracy deltas vs.\ stateless with 95\% question-cluster-bootstrap CIs. Every interval crosses zero; the oracle interval touches zero from below.}
\label{fig:cv}
\end{figure}

Table~\ref{tab:cv} and Figure~\ref{fig:cv} confirm the null at double the sample size: MAJ with self-written memory is $+0.6$ points with a CI spanning zero, and oracle memory does not help (its CI touches zero from below, the cleanest single statement that the memory carries no exploitable label signal). This is the negative half of the bracket, and it is now cross-validated rather than resting on one split.
