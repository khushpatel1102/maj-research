...
A stated freeze is a claim, not a proof. We require the proof, per run.
...
Table~\ref{tab:clean} shows the central result: no configuration outperforms the stateless baseline. MAJ vs.\ stateless is $-5.0$ points (bootstrap CI $[-12.5, +1.3]$, McNemar $p{=}0.29$); MCTS-Judge vs.\ stateless is exactly $0.0$; MCTS-Judge+Memory vs.\ stateless is $+1.8$ ($p{=}0.80$). Every confidence interval in Table~\ref{tab:clean} overlaps the baseline, and every paired bootstrap interval on a mode-versus-mode difference crosses zero. The 6--10 point gain from the conventional protocol is gone. We changed the measurement, not the system.
...
Three run series appear in this paper, each with its own stateless baseline (primary series 70.0\%, \S\ref{sec:results}; the later instrumented run 66.2\%, this section and \S\ref{sec:channels}; grouped CV 68.1\%, \S\ref{sec:cv}).

The $\approx4$-point baseline drift sits inside the cross-seed band of \S\ref{sec:results}; every comparison below is therefore a \emph{paired} test on identical items, never a difference of two headline numbers.
...
\]
with equality if and only if there is a measurable selection $y^{*}(X) \in \arg\max_y P(Y{=}y\mid X)$ that remains optimal given $(X,M)$ almost surely. (A class that merely ties given $X$ does not suffice: if $M$ breaks the tie, the inequality is strict.) In particular, equality holds whenever $Y \perp M \mid X$.
\end{proposition}

This is a standard consequence of the comparison of statistical experiments \citep{blackwell1951, blackwell1953}, stated for orientation, not as a new result: memory can raise the \emph{achievable} accuracy only insofar as the label posterior given $(X,M)$ differs from the one given $X$ alone.
...
\midrule
Primary series (clean, oracle, poison) & \texttt{benchmark\_leakage\_free.py} & \texttt{leakage\_free\_*}, \texttt{lf\_*}, audits \\
Multi-seed replication & same, \texttt{--seed} & \texttt{lf\_*\_seed\{7,123\}*} \\
Memory-use controls & \texttt{exp2\_memory\_controls.py} & \texttt{exp2\_*} \\
$2\times2$ leakage decomposition & \texttt{exp1\_2x2\_leakage.py} & \texttt{exp1\_*} (+dose logs) \\
Grouped five-fold CV & \texttt{exp3\_grouped\_cv.py} & \texttt{exp3\_cv\_*} \\
Threshold (leniency) test & \texttt{exp4\_leniency\_anchor.py} & \texttt{exp4\_*} (retrieval logs) \\
Length / equivalence / sensitivity & \texttt{round3\_analyses.py} & \texttt{round3\_analyses.json} \\
Audit mutation tests & \texttt{test\_audit\_mutation.py} & 7/7 passing at the commit above \\
\bottomrule
\end{tabular}
\caption{Map from every reported experiment to its driver script and released artifacts. Scripts live under \texttt{experiments/} (primary series at the repository root; mutation tests under \texttt{tests/}); per-item records under \texttt{results/}. Splits are reproduced by seed (Appendix~\ref{app:config}); prompts are in \texttt{src/prompts.py}. Each audited run also emits a machine-readable record with the before/after snapshots (per-type counts and state fingerprint) and their diff; a run is leakage-free only if the diff is empty and the fingerprints match.}
\label{tab:manifest}
\end{table}
...
Graph store & Neo4j Community server 5.x (June runs); 5.26.28 (verification environment); Python driver \texttt{neo4j} 5.15.0 \\
